var contentArray = {
    'Default Page': './pages_psyeco/default/default.md',
    '生活哲学': {
        '为什么要哲学?': './pages_psyeco/life-psy/1.md',
        '精神世界': {
            'HSP高敏感人群': './pages_psyeco/life-psy/inner/1.md',
            '过度思考Overthink': './pages_psyeco/life-psy/inner/2.md',
            '人生三大困境-孤独': './pages_psyeco/life-psy/inner/3.md',
            '空': './pages_psyeco/life-psy/inner/4.md',
            '宁静致远': './pages_psyeco/life-psy/inner/5.md'
        },
        '成熟': {
            '宁静致远': './pages_psyeco/life-psy/inner/5.md',
            '对待爱情': './pages_psyeco/life-psy/maturity/0.md',
            '工作-心态-健康': './pages_psyeco/life-psy/maturity/1.md',
            '不要犹豫,Just Do It': './pages_psyeco/life-psy/maturity/3.md',
            '如何充实自己': './pages_psyeco/life-psy/maturity/4.md',
            '正视爱与性|与异性相处': './pages_psyeco/life-psy/maturity/5.md',
            '放开': './pages_psyeco/life-psy/maturity/6.md',
            '友善|感恩|热情': './pages_psyeco/life-psy/maturity/7.md',
            '隐藏与表现|谦虚': './pages_psyeco/life-psy/maturity/8.md',
            '自爱|自信|自强': './pages_psyeco/life-psy/maturity/9.md',
            '行动大于一切': './pages_psyeco/life-psy/maturity/10.md',
            '物质是首要的': './pages_psyeco/life-psy/maturity/11.md',
            '脱敏': './pages_psyeco/life-psy/maturity/12.md',
            '有底线,坚持底线': './pages_psyeco/life-psy/maturity/13.md',
            '论诱惑和专注': './pages_psyeco/life-psy/maturity/14.md',
            '坚持自己的路': './pages_psyeco/life-psy/maturity/15.md',
        },
        '关于学习': {
            '高效学习': './pages_psyeco/life-psy/study/1.md',
            '知识有什么用': './pages_psyeco/life-psy/study/2.md',
            '连续时间的重要性': './pages_psyeco/life-psy/study/3.md',
            '阅读文献的正确姿势': './pages_psyeco/life-psy/study/4.md',
            '论应试': './pages_psyeco/life-psy/study/5.md'
        }
    },
    '音乐': {
        '进阶乐理': {
            '和弦原理': './pages_psyeco/music/chords/1.guitar_chords.md',
            '调式色彩': './pages_psyeco/music/chords/2.tonal_color.md'
        },
        '音乐鉴赏': {
            '想起了你': './pages_psyeco/music/enjoy/1.md',
            '小镇姑娘': './pages_psyeco/music/enjoy/2.md',
        },
        '编曲实战': {
            '蝶恋花': 'null'
        }
    },
    '诗儿词儿': {

    },
    '博弈论': {
        '博弈论概述': './pages_psyeco/game_theory/10.md',
        '完全信息静态博弈': './pages_psyeco/game_theory/20.md',
    }
};